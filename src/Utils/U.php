...
class U
{
  ...
  {
    $ret = self::readPagesOfAllGenerations('default');
debug($ret);
  }
  ...
  public static function readPagesOfAllGenerations($retrieveCacheConfig = false)
  {
    ...
    for($i = 1; $i <= $maxI; $i++) {
      $generation = $i . '0';
      $ret[] = self::readPages($generation, $retrieveCacheConfig);
      if (!$ret) break;
    }
    return $ret;
  }
  ...
  public static function readPages($generation, $retrieveCacheConfig = false)
  {
    ...
    for($page = 1; $page <= $maxPage; $page++) {
      $ret[] = self::readPage($generation, $page);
      if (!$ret) break;
    }
    return $ret;
  }
  ...
  public static function readImg($element)
  {
    ...
    return self::regReplace($thumbnail, $option);
  }
